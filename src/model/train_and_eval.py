...
from .neural_network import Neural_Net, SIN
...
def instantiate_model(units=[10, 10], activation=SIN(), create_parameter=False):

    model_real = Neural_Net(
        units=units,
        activation=activation,
        input=1,
        output=global_variables.JC_DIM,
...
def train(epochs, params, tfinal, n_time_steps, picture, n_points_loss=None):
...
def train_with_parameter(epochs, params, tfinal, n_time_steps, picture, n_points_loss):

    model_real, model_imag = instantiate_model(create_parameter=True)
    ...
    for _ in tqdm(range(epochs)):
        ...
        hamiltonian = hamiltonian_with_params(
            picture=picture, params=params, coupling_strength=coupling_strength
        )
